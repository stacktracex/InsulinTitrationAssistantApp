...
export const getSuggestedDose = (record: Partial<DailyRecord>, config: TitrationConfig = DEFAULT_CONFIG): { basal: number, breakfast: number, lunch: number, dinner: number } => {
  ...
  const titrateBasal = (bg: number | undefined, current: number = 0) => {
    if (bg === undefined || bg <= 0) return current;
    const r = config.basalRules;
    if (bg > r.fbgHighPlus6) return Math.round(current + 6);
    if (bg >= r.fbgMedPlus4) return Math.round(current + 4);
    if (bg >= r.fbgLowPlus2) return Math.round(current + 2);
    if (bg < r.fbgSafeMin) return Math.round(Math.max(0, current - r.basalDecr));
    return Math.round(current);
  };

  const titratePrandial = (bg: number | undefined, current: number = 0) => {
    if (bg === undefined || bg <= 0) return current;
    const r = config.prandialRules;
    if (bg > r.bgHighPlus4) return Math.round(current + 4);
    if (bg >= r.bgMedPlus2) return Math.round(current + 2);
    if (bg < r.bgSafeMin) return Math.round(Math.max(0, current - r.prandialDecr));
    return Math.round(current);
  };

  return {
    basal: titrateBasal(fbg, curBasal),
    breakfast: titratePrandial(preLunchBG, curBreakfast), // 参考午餐前血糖调整早餐前剂量
    lunch: titratePrandial(preDinnerBG, curLunch),      // 参考晚餐前血糖调整午餐前剂量
    dinner: titratePrandial(bedtimeBG, curDinner),       // 参考睡前血糖调整晚餐前剂量
  };
};
